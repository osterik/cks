...
@test "1. Control plane node upgraded to ${TARGET} and Ready" {
  ...
  cp=$(kubectl get nodes -l node-role.kubernetes.io/control-plane $CTX -o jsonpath='{.items[0].status.nodeInfo.kubeletVersion}' 2>/dev/null)
  rd=$(kubectl get nodes -l node-role.kubernetes.io/control-plane $CTX -o jsonpath='{.items[0].status.conditions[?(@.type=="Ready")].status}' 2>/dev/null)
...
@test "2. All nodes upgraded to ${TARGET} and Ready" {
  ...
  ontarget=$(kubectl get nodes $CTX -o jsonpath='{.items[*].status.nodeInfo.kubeletVersion}' 2>/dev/null | tr ' ' '\n' | grep -c "^${TARGET}$")
