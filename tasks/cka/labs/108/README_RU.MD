...
Во время подготовки окружения на worker создаётся Pod `reserve` с запросами ресурсов `1500m` CPU
и `1200Mi` памяти. Благодаря этому worker становится менее предпочтительным для стандартного
планировщика, если у Pod не задано ограничение на выбор ноды, поэтому в задании 3 необходим явный
`nodeSelector`. Pod `reserve` и его запросы ресурсов являются частью инфраструктуры лабораторной
работы и не должны изменяться.
