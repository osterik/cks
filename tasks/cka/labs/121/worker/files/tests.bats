...
@test "1. ServiceAccount app-sa exists in ns rbac" {
  ...
  NS="-n rbac"
  sa=$(kubectl get sa app-sa $CTX $NS -o jsonpath='{.metadata.name}' 2>/dev/null)
...
@test "2. Role pod-reader (get/list/watch pods) in ns rbac" {
  echo '1' >> /var/work/tests/result/all
  NS="-n rbac"
  res=$(kubectl get role pod-reader $CTX $NS -o jsonpath='{.rules[*].resources[*]}' 2>/dev/null)
  verbs=$(kubectl get role pod-reader $CTX $NS -o jsonpath='{.rules[*].verbs[*]}' 2>/dev/null)
...
@test "3. RoleBinding read-pods binds pod-reader to app-sa" {
  echo '1' >> /var/work/tests/result/all
  NS="-n rbac"
  ref=$(kubectl get rolebinding read-pods $CTX $NS -o jsonpath='{.roleRef.name}' 2>/dev/null)
  subj=$(kubectl get rolebinding read-pods $CTX $NS -o jsonpath='{.subjects[*].name}' 2>/dev/null)
...
@test "4. ClusterRole cr-viewer (get/list nodes)" {
  echo '1' >> /var/work/tests/result/all
  res=$(kubectl get clusterrole cr-viewer $CTX -o jsonpath='{.rules[*].resources[*]}' 2>/dev/null)
  verbs=$(kubectl get clusterrole cr-viewer $CTX -o jsonpath='{.rules[*].verbs[*]}' 2>/dev/null)
...
@test "5. ClusterRoleBinding crb-viewer binds cr-viewer to app-sa" {
  echo '1' >> /var/work/tests/result/all
  ref=$(kubectl get clusterrolebinding crb-viewer $CTX -o jsonpath='{.roleRef.name}' 2>/dev/null)
  sname=$(kubectl get clusterrolebinding crb-viewer $CTX -o jsonpath='{.subjects[*].name}' 2>/dev/null)
  sns=$(kubectl get clusterrolebinding crb-viewer $CTX -o jsonpath='{.subjects[*].namespace}' 2>/dev/null)
...
@test "6. Effective permissions of app-sa are correct" {
  echo '1' >> /var/work/tests/result/all
  NS="-n rbac"
  can_pods=$(kubectl auth can-i get pods --as=$SA $CTX $NS 2>/dev/null)
  can_nodes=$(kubectl auth can-i get nodes --as=$SA $CTX 2>/dev/null)
  cannot_del=$(kubectl auth can-i delete pods --as=$SA $CTX $NS 2>/dev/null || true)
