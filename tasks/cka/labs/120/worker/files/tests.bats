...
@test "3. NodePort service web-np on nodePort 30080" {
  echo '1' >> /var/work/tests/result/all
  NS="-n net"
  t=$(kubectl get svc web-np $CTX $NS -o jsonpath='{.spec.type}' 2>/dev/null)
  np=$(kubectl get svc web-np $CTX $NS -o jsonpath='{.spec.ports[0].nodePort}' 2>/dev/null)
...
@test "5. Ingress web-ing routes web.local to web-svc:80" {
  echo '1' >> /var/work/tests/result/all
  NS="-n net"
  host=$(kubectl get ingress web-ing $CTX $NS -o jsonpath='{.spec.rules[0].host}' 2>/dev/null)
  svc=$(kubectl get ingress web-ing $CTX $NS -o jsonpath='{.spec.rules[0].http.paths[0].backend.service.name}' 2>/dev/null)
  port=$(kubectl get ingress web-ing $CTX $NS -o jsonpath='{.spec.rules[0].http.paths[0].backend.service.port.number}' 2>/dev/null)
...
@test "6. NetworkPolicy deny-all (default-deny Ingress) in ns net" {
  echo '1' >> /var/work/tests/result/all
  NS="-n net"
  types=$(kubectl get netpol deny-all $CTX $NS -o jsonpath='{.spec.policyTypes[*]}' 2>/dev/null)
  sel=$(kubectl get netpol deny-all $CTX $NS -o jsonpath='{.spec.podSelector.matchLabels}' 2>/dev/null)
...
@test "7. JSONPath: nodePort of web-np saved to answers/nodeport.txt" {
  echo '1' >> /var/work/tests/result/all
  NS="-n net"
  exp=$(kubectl get svc web-np $CTX $NS -o jsonpath='{.spec.ports[0].nodePort}' 2>/dev/null | xargs)
