...
@test "1. CRD operators.stable.example.com (group, kind Operator, scope Namespaced)" {
  ...
  grp=$(kubectl get crd operators.stable.example.com $CTX -o jsonpath='{.spec.group}' 2>/dev/null)
  kind=$(kubectl get crd operators.stable.example.com $CTX -o jsonpath='{.spec.names.kind}' 2>/dev/null)
  scope=$(kubectl get crd operators.stable.example.com $CTX -o jsonpath='{.spec.scope}' 2>/dev/null)
...
@test "2. Helm release prom in namespace monitoring" {
  ...
  ns=$(kubectl get ns monitoring $CTX -o jsonpath='{.metadata.name}' 2>/dev/null)
...
@test "3. Kustomize overlay applied: deployment kapp (3 replicas) in kustns" {
  echo '1' >> /var/work/tests/result/all
  NS="-n kustns"
  rep=$(kubectl get deploy kapp $CTX $NS -o jsonpath='{.spec.replicas}' 2>/dev/null)
  if [[ "$rep" == "3" ]]; then
    echo '1' >> /var/work/tests/result/ok; result=0
