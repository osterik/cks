...
@test "1. Namespace drill exists" {
  ...
  ns=$(kubectl get ns drill $CTX -o jsonpath='{.metadata.name}' 2>/dev/null)
...
@test "2. Deployment api (ns drill) has 3 ready replicas" {
  echo '1' >> /var/work/tests/result/all
  NS="-n drill"
  r=$(kubectl get deploy api $CTX $NS -o jsonpath='{.status.readyReplicas}' 2>/dev/null)
...
@test "4. ConfigMap cfg (APP_COLOR=blue) and Secret sec (key PASSWORD) exist in drill" {
  echo '1' >> /var/work/tests/result/all
  NS="-n drill"
  color=$(kubectl get cm cfg $CTX $NS -o jsonpath='{.data.APP_COLOR}' 2>/dev/null)
  pass=$(kubectl get secret sec $CTX $NS -o jsonpath='{.data.PASSWORD}' 2>/dev/null)
...
@test "5. JSONPath: node kubelet versions saved to answers/versions.txt" {
  echo '1' >> /var/work/tests/result/all
  exp=$(kubectl get nodes $CTX -o jsonpath='{.items[*].status.nodeInfo.kubeletVersion}' 2>/dev/null | xargs)
...
@test "6. JSONPath: container images in ns drill saved to answers/images.txt" {
  echo '1' >> /var/work/tests/result/all
  NS="-n drill"
  exp=$(kubectl get pods $CTX $NS -o jsonpath='{.items[*].spec.containers[*].image}' 2>/dev/null | tr ' ' '\n' | sort | xargs)
...
@test "7. JSONPath: ClusterIP of api-svc saved to answers/api-ip.txt" {
  echo '1' >> /var/work/tests/result/all
  NS="-n drill"
  exp=$(kubectl get svc api-svc $CTX $NS -o jsonpath='{.spec.clusterIP}' 2>/dev/null | xargs)
