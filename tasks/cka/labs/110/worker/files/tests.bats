...
@test "1. nginx-resolver pod + service, DNS records saved to files" {
  ...
  NS="-n default"
  svc=$(kubectl get svc nginx-resolver-service $CTX $NS -o jsonpath='{.metadata.name}' 2>/dev/null)
  eps=$(kubectl get endpoints nginx-resolver-service $CTX $NS -o jsonpath='{.subsets[*].addresses[*].ip}' 2>/dev/null | wc -w)
...
@test "2. Ingress for /cat (ns cat, rewrite-target, backend cat)" {
  echo '1' >> /var/work/tests/result/all
  NS="-n cat"
  path=$(kubectl get ingress $CTX $NS -o json 2>/dev/null | jq -r '.items[0].spec.rules[0].http.paths[] | select(.path=="/cat") | .path' | head -1)
  bk=$(kubectl get ingress $CTX $NS -o json 2>/dev/null | jq -r '.items[0].spec.rules[0].http.paths[] | select(.path=="/cat") | .backend.service.name' | head -1)
  ann=$(kubectl get ingress $CTX $NS -o json 2>/dev/null | jq -r '.items[0].metadata.annotations["nginx.ingress.kubernetes.io/rewrite-target"]')
...
@test "4. Migration: Gateway shop-gw + HTTPRoute shop-route equivalent to Ingress (ns gw)" {
  echo '1' >> /var/work/tests/result/all
  NS="-n gw"
  gwcls=$(kubectl get gateway shop-gw $CTX $NS -o jsonpath='{.spec.gatewayClassName}' 2>/dev/null)
  host=$(kubectl get httproute shop-route $CTX $NS -o jsonpath='{.spec.hostnames[0]}' 2>/dev/null)
  path=$(kubectl get httproute shop-route $CTX $NS -o json 2>/dev/null | jq -r '.spec.rules[0].matches[0].path.value' 2>/dev/null)
  bk=$(kubectl get httproute shop-route $CTX $NS -o json 2>/dev/null | jq -r '.spec.rules[0].backendRefs[0].name' 2>/dev/null)
  parent=$(kubectl get httproute shop-route $CTX $NS -o jsonpath='{.spec.parentRefs[0].name}' 2>/dev/null)
