...
@test "1. ReplicaSet rs-app2223 (ns rsapp) has 2 ready replicas" {
  ...
  NS="-n rsapp"
  ready=$(kubectl get rs rs-app2223 $CTX $NS -o jsonpath='{.status.readyReplicas}' 2>/dev/null)
  if [[ "$ready" == "2" ]]; then
...
@test "2. Service svc-broken (ns tsvc) now has endpoints" {
  echo '1' >> /var/work/tests/result/all
  NS="-n tsvc"
  eps=$(kubectl get endpoints svc-broken $CTX $NS -o jsonpath='{.subsets[*].addresses[*].ip}' 2>/dev/null | wc -w)
  if [[ "$eps" -ge 1 ]]; then
    echo '1' >> /var/work/tests/result/ok; result=0
...
@test "3. Deployment capp (ns cfgapp) is ready (ConfigMap fixed)" {
  echo '1' >> /var/work/tests/result/all
  NS="-n cfgapp"
  ready=$(kubectl get deploy capp $CTX $NS -o jsonpath='{.status.readyReplicas}' 2>/dev/null)
  if [[ "$ready" -ge 1 ]]; then
    echo '1' >> /var/work/tests/result/ok; result=0
